...
export default {
    ...
    getPortfolioCurrencies() {
        let url = buildUrl('portfolio/currencies');
        
        return axios.get(url, buildOptions()).then(res => res.data);
    },
    createAddress(record) {
        let url = buildUrl('portfolio');
        
        return axios.post(url, record, buildOptions()).then(res => res.data);
    },
    deleteAddress(record) {
        let url = buildUrl(`portfolio/${record.id}`);
